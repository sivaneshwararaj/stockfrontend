<script lang="ts">
  ...
  onMount(async () => {
    CountrySegmentation = (
      await import("$lib/components/CountrySegmentation.svelte")
    ).default;
    SectorSegmentation = (
      await import("$lib/components/SectorSegmentation.svelte")
    ).default;
  });
      ...
      if ($realtimePrice !== null) {
        ...
        for (let i = length - 1; i >= 0; i--) {
          if (!isNaN(oneDayPrice[i]?.close ?? oneDayPrice[i]?.value)) {
            $currentPortfolioPrice =
              oneDayPrice[i]?.close ?? oneDayPrice[i]?.value;
            break;
          }
        }
      }
  ...
  async function handleCrosshairMove({ detail: param }) {
    if (
      ...
      try {
        ...
        const formattedDate =
          displayData === "1D" || displayData === "1W" || displayData === "1M"
            ? date.toLocaleString("en-GB", options).replace(/\//g, ".")
            : date
...
<svelte:head>
  ...
  <meta
    name="description"
    content={`Get a real-time ${$displayCompanyName} (${$etfTicker}) stock price quote with breaking news, financials, statistics, charts and more.`}
  />
  <!-- Other meta tags -->
  <meta
    property="og:title"
    ...
    <div class="md:flex md:justify-between md:divide-x md:divide-slate-800">
      ...
      <div class="pb-12 md:pb-20 w-full sm:pr-6 xl:pr-0">
        <div class="xl:pr-10">
          ...
          <Lazy>
            <div
              class="w-full mt-10 m-auto sm:p-6 {data?.getWhyPriceMoved
                ?.length !== 0
                ? ''
                : 'hidden'}"
            >
          ...
          <Lazy>
            <div
              class="w-full mt-10 sm:mt-0 m-auto sm:p-6 {!$optionComponent
                ? 'hidden'
                : ''}"
            >
          ...
          <Lazy>
            <div
              class="w-full mt-10 sm:mt-0 m-auto sm:p-6 {!$optionsNetFlowComponent
                ? 'hidden'
                : ''}"
            >
          ...
          <div
            class="w-full mt-10 sm:mt-5 m-auto sm:pl-6 sm:pb-6 sm:pt-6 {geographicList?.length !==
            0
              ? ''
              : 'hidden'}"
          >
            {#if CountrySegmentation}
              <CountrySegmentation {geographicList} />
            {/if}
          </div>
          ...
          <div
            class="w-full pt-10 sm:p-6 m-auto {sectorList?.length === 0
              ? 'hidden'
              : ''}"
          >
            {#if SectorSegmentation}
              <SectorSegmentation {sectorList} />
            {/if}
          </div>
