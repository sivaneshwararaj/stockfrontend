<script lang="ts">
  ...
  const subsectionTitles = ['Overview', 'Data Center', 'Gaming', 'Visualization', 'Automotive', 'OEM & Other'];
  ...
  function changeSubSection(state) {
    ...
    const path = state === "overview" ? "/metrics" : `/metrics/${sectionMap[state]}`;
    // Navigate programmatically using SvelteKit's goto function
  }

  function getHref(section) {
    const path = section === "overview" ? "/metrics" : `/metrics/${sectionMap[section]}`;
  ...
  $: {
    if ($page?.url?.pathname) {
      ...
      const foundSection = parts.find(part => part && Object.values(sectionMap).includes(part));
...
<section class="w-auto max-w-5xl bg-[#09090B] overflow-hidden text-black h-full mb-40">
  <div class="m-auto h-full overflow-hidden">
    <main class="w-fit sm:w-full sm:max-w-2xl">
      <div class="m-auto">
        <div class="-ml-2 sm:ml-8 w-screen sm:w-full {$screenWidth < 640 ? 'overflow-auto scrollbar' : ''} mb-2">
            ...
            {#each subsectionTitles as title}
              ...
              <li class="cursor-pointer flex flex-col items-center">
                <a
                  ...
                  class="px-2 text-sm sm:text-[1rem] whitespace-nowrap font-medium text-gray-400 sm:hover:text-white {displaySubSection === sectionKey ? 'text-white' : 'bg-[#09090B]'}"
                >
                  {title}
                </a>
                <div class="{displaySubSection === sectionKey ? 'bg-[#75D377]' : 'bg-[#09090B]'} mt-1 h-[3px] rounded-full w-[3rem]" />
              </li>
            {/each}
          </ul>
        </div>
