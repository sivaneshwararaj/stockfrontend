...
<svelte:head>
  ...
  <title>
    ...
    {$displayCompanyName} ({$cryptoTicker}) Statistics & Valuation Metrics ·
    stocknear
  </title>
  <meta
    name="description"
    content={`Detailed statistics for ${$displayCompanyName} (${$cryptoTicker}) stock, including valuation, metrics, financial numbers, share information and more.`}
  />
          ...
          {#if $cryptoTicker in quantStats && Object.keys(quantStats[$cryptoTicker]).length > 0}
            ...
            >
              Time Period between {new Date(
                quantStats[$cryptoTicker?.toUpperCase()]["Start Period"],
              ...
              -
              {new Date(
                quantStats[$cryptoTicker?.toUpperCase()]["End Period"],
              ).toLocaleString("en-US", {
                month: "long",
                day: "numeric",
                year: "numeric",
