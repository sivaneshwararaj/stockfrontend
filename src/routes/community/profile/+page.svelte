<script lang="ts">
  ...
  async function updateAvatar(event) {
    ...
    try {
      ...
      const { avatar } = await pb
        .collection("users")
        .update(data?.user?.id, serialize(postData));
    ...
    */
      ...
      if (error instanceof z.ZodError) {
        ...
        errorAvatar =
          zodErrors.find((err) => err.path[0] === "avatar")?.message ?? "";
  ...
  async function updatePersonalData(event) {
    ...
    try {
      ...
      const { username } = await pb
        .collection("users")
        .update(data?.user?.id, cleanedData);
      ...
      if (error instanceof z.ZodError) {
        ...
        errorUsername =
          zodErrors?.find((err) => err.path[0] === "username")?.message ?? "";
  ...
  function isModerator(data, moderators) {
    return moderators?.some(
      (moderator) => data?.user?.id === moderator?.expand?.user?.id,
    );
  }
...
<svelte:head>
  <title>
    ...
    {data?.user?.username} · stocknear</title
  >
  <meta charset="utf-8" />
  ...
  <meta
    name="description"
    content="Explore {data?.user
      ?.username}'s latest posts, comments, and notebooks on stocknear. Discover new insights and connect with other users in the stocknear community."
  />
  <!-- Other meta tags -->
        ...
        <section>
          <div class="w-full max-w-7xl m-auto sm:px-20 ml-auto">
            <div class="w-full flex flex-row">
              <!--Start Profile Pic-->
              <div
                class="flex items-center justify-start mb-5 w-screen sm:w-full sm:bg-[#141417] h-48 sm:rounded-xl sm:border sm:border-gray-700 sm:hover:border-gray-600"
              >
                <form on:submit={handleAvatar} class="ml-5">
                  <input
                    type="file"
                    name="avatar"
                    id="avatar"
                  ...
                  <label
                    for="avatar"
                    class="avatar w-20 h-20 sm:w-24 sm:h-24 rounded-full sm:hover:cursor-pointer"
                  >
                    <label
                      for="avatar"
                      class="absolute z-10 -bottom-0.5 -right-0.5 sm:hover:cursor-pointer"
                    >
                ...
                <div class="lg:pl-5 z-20 h-full">
                  ...
                  <div class="space-y-6 ml-4">
                    ...
                    >
                      ...
                      <div
                        class="flex flex-col items-start {data?.user?.tier ===
                        'Pro'
                          ? 'mt-3'
                ...
                <div class="md:pr-6 lg:pr-10 mt-6">
                  ...
                  {:else if showTab === "settings"}
                    <div class="flex flex-col w-full h-screen p-2">
                      <div class="w-full">
                        <div
                          class="tabs pb-10 pt-10 flex flex-row items-center"
                        >
                          <label
                            on:click={() => changeTab("personalData")}
                            class="tab tab-sm pr-5 text-gray-400 hover:text-white {settingsTab ===
                            'personalData'
                              ? 'rounded text-white bg-[#333333]'
                              : 'bg-[#09090B]'}">Personal Data</label
                          >
                          <label
                            on:click={() => changeTab("changePassword")}
                            class="tab tab-sm pl-5 text-gray-400 hover:text-white {settingsTab ===
                            'changePassword'
                              ? 'rounded text-white bg-[#333333]'
                              : 'bg-[#09090B]'}">Change Password</label
                          >
                        </div>
                  ...
                  {:else if showTab === "subscription"}
                    <div
                      class="flex flex-col sm:flex-row overflow-hidden pl-5 p-0 sm:p-5"
                    >
                      <div
                        class="flex flex-col justify-start items-start w-full text-white"
                      >
                        <h2
                          class="text-xl sm:text-2xl font-bold text-start mt-5 mb-5"
                        >
                          Manage Subscription
                        </h2>
                        <div class="flex flex-row items-center">
                          ...
                          <div class="ml-2 flex flex-row items-center">
                            ...
                            <span
                              class="ml-2 text-[1rem] text-slate-200 font-medium"
                            >
                        ...
                        {:else if data?.user?.freeTrial === true}
                          Your trial will end on {trialEndDate?.toLocaleDateString(
                            "en-GB",
                        ...
                        <div
                          class="flex flex-col justify-start items-start mt-8"
                        >
                          ...
                          <span class="text-[1rem]">
                            ...
                            {:else}
                              {[
                                ...
                                "On Trial",
                                "Cancelled",
                              ]?.includes(subscriptionData?.status_formatted)
                        ...
                        {#if subscriptionData?.status_formatted === "Active" || subscriptionData?.status_formatted === "On Trial"}
                          ...
                          >
                            <label
                              ...
                              class="cursor-pointer text-white border border-gray-600 sm:hover:bg-[#27272A] bg-opacity-[0.5] text-sm sm:text-[1rem] px-4 py-2 rounded-lg mt-5"
                            >
                              Cancel Subscription
                            </label>
                            {#if subscriptionData?.product_name?.includes("Monthly")}
                              <label
                                ...
                                class="sm:ml-3 {subscriptionData?.card_brand !==
                                  null &&
                                subscriptionData?.card_brand?.length !== 0
                                  ? 'cursor-pointer'
                                ...
                                subscriptionData?.card_brand?.length !== 0
                                  ...
                                  : 'bg-gray-600 opacity-[0.8] text-white'} text-sm sm:text-[1rem] px-4 py-2 rounded-lg mt-5"
                              >
                        ...
                        {:else if subscriptionData?.status_formatted === "Paid" && !subscriptionData?.first_order_item?.product_name === "Pro Subscription (Lifetime Access"}
                          <span class="text-white mt-5">
                            Please wait a moment; you will be updated to Pro in
                            a second.
                          </span>
                        {:else if data?.user?.freeTrial || subscriptionData?.first_order_item?.product_name === "Pro Subscription (Lifetime Access)"}{:else}
