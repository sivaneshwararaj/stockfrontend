<script lang="ts">
  ...
  const handleUpvote = async (event) => {
    ...
    $postVote = {
      id: postId,
      upvote: upvoteCounter[postId],
      downvote: downvoteCounter[postId],
      upvoteClicked: upvoteButtonClicked[postId],
      downvoteClicked: downvoteButtonClicked[postId],
    };
  ...
  const handleCopyLink = async () => {
    ...
    await navigator.clipboard.writeText(
      "https://stocknear.com/community/post/" + post.id,
    );
  ...
  onMount(async () => {
    ...
    userAlreadyVoted = post?.expand["alreadyVoted(post)"]?.some(
      (item) => item?.user === data?.user?.id,
    );

    if (userAlreadyVoted) {
      upvoteButtonClicked[post.id] =
        post?.expand["alreadyVoted(post)"]?.find(
          (item) => item?.user === data?.user?.id,
        )?.type === "upvote";
      downvoteButtonClicked[post.id] =
        post?.expand["alreadyVoted(post)"]?.find(
          (item) => item?.user === data?.user?.id,
        )?.type === "downvote";
    } else {
  ...
  afterNavigate(({ from }) => {
    previousPage = from?.url.pathname?.startsWith("/community/create-post")
      ? "/community"
      : from?.url.pathname || "/community";
  });
  ...
  function updateVote(postVote) {
    ...
    if (item) {
      ...
      if (
        ...
        item?.expand["alreadyVoted(post)"]?.length > 0
      ) {
        // Find the vote entry for the current user, if it exists
        const userVote = item?.expand["alreadyVoted(post)"]?.find(
  ...
  function addCommentToParent(comments, newComment) {
    ...
    function findAndAddParent(commentsList, newComment) {
      for (let comment of commentsList) {
        ...
        if (
          comment.children &&
          findAndAddParent(comment.children, newComment)
        ) {
          return true; // Indicate that the comment has been added
        }
      }
  ...
  function updateCommentInList(newObject, list) {
    ...
    function findAndReplaceComment(commentsList, newComment) {
      for (let comment of commentsList) {
        ...
        if (
          comment.children &&
          findAndReplaceComment(comment.children, newComment)
        ) {
          return true; // Indicate that the comment has been updated
        }
      }
  ...
  $: {
    if ($commentAdded?.length !== 0) {
      comments = sortCommentsByDate(
        addCommentToParent(comments, $commentAdded),
      );
...
<svelte:head>
  ...
  <meta
    name="description"
    content="Your daily dose of stock market funny memes, GIFs, videos and weird news stories. We deliver hundreds of new stock market memes daily."
  />
  <!-- Other meta tags -->
...
<div
  class="overflow-hidden flex flex-row item-start w-full lg:mt-5 relative max-w-3xl sm:max-w-screen-2xl lg:px-5 sm:pb-40"
>
  <!--Start Voting-->
  <div
    style="top: 4rem;"
    class="hidden lg:flex flex-col items-center relative h-fit sticky z-20 lg:mr-3"
  >
    <!--Start Upvote-->
    ...
    <!--End Upvote-->
    <label
      class="px-6 py-4 w-14 rounded-lg bg-[#27272A] text-[1rem] text-bold text-white"
    >
      {typeof upvoteCounter[post?.id] === "number" ||
      typeof downvoteCounter[post?.id] === "number"
  ...
  <div
    class="w-full bg-[#141417] max-w-5xl border sm:hover:border-slate-800 border-slate-800 rounded-none sm:rounded-lg"
  >
    <!-- Start Header -->
    <div
      style="top: 0.8rem;"
      class="sm:rounded-lg absolute h-12 sticky z-20 bg-[#141417] w-full {isScrolled &&
      $screenWidth < 640
        ? 'border-b border-gray-700 ease-in'
        : 'ease-out'}"
    >
      <div class="flex flex-row items-center justify-between w-full pt-3">
        <a href={previousPage} class="absolute left-2 sm:left-4 sm:top-4">
          <svg
            class="w-5 h-5 inline-block sm:mr-1 sm:-mt-0.5"
        ...
        <div class="md:flex md:justify-between md:divide-x md:divide-slate-800">
          ...
          <div class="md:grow pb-12 md:pb-20">
            ...
            <div class="w-full m-auto mt-10 pb-20">
              <a
                href={"/community/user/" + post?.expand?.user?.id}
                class="flex flex-row items-center justify-start ml-3 pb-5 mr-auto"
              >
                <label
                  class="avatar w-10 h-10 flex-shrink-0 text-white text-xs sm:text-sm ml-1"
                >
                  <img
                ...
                {#if isLoaded}
                  {#if isModerator(post?.expand?.user?.id)}
                    <svg
                      class="inline-block ml-1 w-3.5 h-3.5"
                      xmlns="http://www.w3.org/2000/svg"
                      viewBox="0 0 512 512"
                      ><path
                        fill="#75d377"
              ...
              <h1
                class="text-xl sm:text-2xl pl-3 pr-3 font-semibold mt-4 text-[#D7DADC] cursor-auto"
              >
                {#if post?.postType !== "link"}
                  {post?.title}
                {/if}
              ...
              {#if videoId === null}
                {#if post?.postType === "link"}
                  ...
                  <div class="flex flex-col sm:flex-row items-start pb-4">
                      ...
                      {#if post?.thumbnail}
                        ...
                        >
                          <img
                            ...
                            class="w-screen sm:w-40 lg:w-48 max-h-[250px] sm:max-h-[120px] rounded-lg sm:rounded-2xl mr-4 mb-2"
                            alt="news image"
                          />
                        </a>
                      {/if}
                    ...
                    <div
                      class="flex-grow {post?.thumbnail ? '' : 'ml-1 sm:ml-5'} "
                    >
                      <h1
                        class="text-start text-xl sm:text-2xl text-[#D7DADC] font-semibold mb-2 flex-shrink ml-2 sm:ml-0"
                      >
                        {post?.title}
                      </h1>
                      {#if post?.description !== "undefined"}
                        <span
                          class="w-full flex justify-start items-center text-start text-[#D7DADC] text-sm sm:text-[1rem] mb-2 flex-shrink ml-2 mt-2 sm:ml-0"
                        >
                      ...
                      <a
                        href={post?.link}
                        target="_blank"
                        class="mt-2 text-[1rem] text-gray-400 font-semibold ml-2.5 sm:ml-0"
                      >
                        {new URL(post?.link)?.hostname.replace("www.", "")}
                        <svg
                          class="ml-1 w-4 h-4 inline-block -mt-0.5"
                          viewBox="0 0 24 24"
                          fill="none"
                          xmlns="http://www.w3.org/2000/svg"
                ...
                {:else}
                  {#if post?.thumbnail?.length !== 0}
                    {#if !post?.thumbnail?.toLowerCase()?.includes("mp4")}
                      ...
                      <label
                        class="mt-10 bg-[#000] m-auto flex justify-center items-center rounded-xl"
                      >
                        <img
                          src={getImageURL(
                            post?.collectionId,
                          ...
                          alt="image"
                          class="relative bg-center m-auto object-center w-auto relative max-h-[520px] sm:max-h-[700px]"
                        />
                      </label>
                    {:else}
                      <label
                      ...
                      >
                        <!--
                                                  ...
                                                  -->
                        <div
                          class="relative m-auto {$screenWidth < 640
                            ? 'w-screen'
                            : 'w-fit max-w-[800px] max-h-[700px]'}"
                        >
                          <VideoPlayer
                            src={getImageURL(
                              post?.collectionId,
                            ...
                            hideProgressbar={$screenWidth > 640 ? true : false}
                          />
                        </div>
                      </label>
                    {/if}
              ...
              {:else}
                ...
                <div
                  class="mt-10 m-auto flex justify-center items-center mb-10"
                >
                  <iframe
              ...
              <p
                class="mt-5 text-lg whitespace-normal {videoId !== null
                  ? 'hidden'
                  : ''} "
              >
                {#if post?.postType !== "link"}
                  <div
                    class="p-3 text-sm sm:text-[1rem] whitespace-pre-line text-[#D7DADC]"
                  >
                    {@html post?.description}
                  </div>
                {/if}
              ...
              <div class="flex flex-row items-center">
                {#if post?.tagline?.length !== 0}
                  <label
                    class="bg-[#D3D6DA] ml-3 mr-3 text-xs h-fit text-[#171717] font-medium p-2 rounded"
                  >
                ...
                <!--Start Tags-->
                <div
                  class="flex flex-row {post?.tagline ? '' : 'ml-3'} mr-auto"
                >
                    ...
                    {#each $tagList as tag}
                      {#each post?.tagTopic as tagTopic}
                        {#if tag?.name === tagTopic}
                          <label
                            class="bg-[#D3D6DA] mr-3 text-xs h-fit text-[#171717] font-medium p-2 rounded"
                          >
              ...
              <div
                class="flex flex-row items-center justify-start ml-8 mt-8 mb-3 w-full"
              >
                <form
                  on:submit={handleUpvote}
                  class="-ml-4 lg:hidden mt-1 mr-2"
                >
                  <input type="hidden" name="postId" value={post?.id} />
                  {#if !data?.user}
                    <div class="w-full">
                ...
                <form
                  on:submit={handleDownvote}
                  class="ml-1 lg:hidden mt-1 mr-2"
                >
                  <input type="hidden" name="postId" value={post?.id} />
                  {#if !data?.user}
                ...
                <div
                  class="ml-6 lg:hidden flex justify-center items-center rounded-md"
                >
                  <svg
                    class="w-4 h-4 mt-1 inline-block mr-2 lg:mr-3"
                    xmlns="http://www.w3.org/2000/svg"
                  ...
                  >
                  <span
                    class="ml-2 text-sm text-white font-semibold lg:font-medium"
                  >
                    {numberOfComments}
                  </span>
                </div>
              ...
              {#if !loadTextEditor}
                <div class="flex justify-center items-center h-80">
                  <div class="relative">
                    <label
                      class="bg-[#141417] rounded-xl h-14 w-14 flex justify-center items-center absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2"
                    >
                      <span
              ...
              {:else}
                ...
                <div class="w-full max-w-6xl mt-5">
                  {#if data?.user}
                    <div class="w-full sm:w-5/6 mb-5">
                      <TextEditor {data} postId={post.id} commentId="" />
                      <!--Submit button in TextEditor component-->
                    </div>
                  {:else}
                    <div
                      class="text-sm sm:text-md text-white bg-[#4B5563] bg-opacity-[0.2] w-full sm:w-fit flex items-center justify-start sm:justify-center pl-2 sm:pl-5 pr-0 sm:pr-5 h-16 rounded-none sm:rounded-lg"
                    >
                      <svg
                        class="w-5 h-5 inline-block flex-shrink-0 mr-2"
                        xmlns="http://www.w3.org/2000/svg"
                        viewBox="0 0 256 256"
                        ><path
                          fill="#fff"
                      ...
                      <label
                        for="userLogin"
                        class="text-blue-400 hover:text-white ml-1 mr-1 font-medium underline"
                        >sign up</label
                      >
                ...
                {#if numberOfComments === 0}
                  <p
                    class="text-center sm:text-start text-slate-100 sm:pl-2 pr-2 text-sm sm:text-md pb-20"
                  >
                    No comments available yet. Be the first and open the
                    discussion! 🚀
                  </p>
                {/if}
                ...
                <div class="mt-10 ml-2">
                  {#each comments as comment}
                    <CommentSection
                      ...
                      userAlreadyVoted={comment?.expand[
                        "alreadyVoted(comment)"
                      ]?.some((item) => item?.user === data?.user?.id)}
                    />
                  {/each}
                </div>
                <!--End comment section-->
...
{#if data?.user}
  ...
  <dialog id="deletePostModal" class="modal modal-bottom sm:modal-middle">
    ...
    <div
      class="modal-box w-full bg-[#141417] sm:border sm:border-slate-600 overflow-hidden"
    >
      <h3
        class="font-bold text-xl mb-5 pt-5 text-white m-auto w-3/4 text-center"
      >
      ...
      >
        <label
          ...
          class="sm:ml-3 btn bg-[#FF3131] hover:bg-[#ff4343] text-white btn-md w-full rounded-lg text-white font-bold text-md"
        >
          Proceed
        </label>
      </div>
