...
const markLineData = rawData.fomcData
  ...
  tableList = rawData.fomcData?.sort((a,b) => new Date(b?.date) - new Date(a?.date))
    ...
    <section class="overflow-hidden text-white h-full ">
        <main class="overflow-hidden ">
            ...
            <div class="flex flex-row items-center">
                <label for="fomcInfo" class="mr-1 cursor-pointer flex flex-row items-center text-white text-xl sm:text-3xl font-bold">
                 FOMC Impact
                </label>
                <InfoModal
            ...
            <div class="mt-2 pb-8 sm:pb-2 rounded-lg bg-[#09090B] sm:bg-[#09090B]">
               ...
               <h2 class="mt-10 mr-1 flex flex-row items-center text-white text-2xl font-bold mb-3">
                    ...
                    <table class="table table-sm table-compact w-full">
                      <thead>
                        <tr class="border-b border-[#27272A]">
                          ...
                          <th class="text-white font-semibold text-sm sm:text-[1rem] text-end bg-[#09090B]">Actual</th>
                          <th class="text-white font-semibold text-sm sm:text-[1rem] text-end bg-[#09090B]">% Price Change</th>
                        </tr>
                      </thead>
                      <tbody>
                        {#each (showFullStats ? tableList?.slice(0,10) : tableList?.slice(0,3)) as item,index}
                        <tr class="border-y border-gray-800 odd:bg-[#27272A] {index === 2 && !showFullStats && tableList?.length > 3 ? 'opacity-[0.5]' : '' } sm:hover:bg-[#245073] sm:hover:bg-opacity-[0.2] bg-[#09090B] border-b-[#09090B]">
                            ...
                            <td class="text-white font-normal text-end text-sm sm:text-[1rem] whitespace-nowrap">
                                 {#if item?.changePercentage >=0}
                                    <span class="text-[#37C97D]">+{item?.changePercentage?.toFixed(2)}%</span>
                                {:else}
                                    <span class="text-[#FF2F1F]">{item?.changePercentage?.toFixed(2)}% </span> 
                                {/if}
                            </td>
